...
Data files are not stored in this GitHub repository. Raw data and generated data outputs are archived on Zenodo:

[https://doi.org/10.5281/zenodo.19674252](https://doi.org/10.5281/zenodo.19674252)

To run the workflow, download the data from Zenodo and place the main input file here:

`data/all_locations.csv`

Derived `.rds` files are generated by the workflow and are ignored by Git.

## Reproducibility

### Data import
The required tracking data is not included in this repository due to size and archiving considerations.
...
Main data outputs, generated locally under `data/`, include:
...
The recent workflow also writes generated data outputs under `data/recent_2_years/`. These local data outputs are ignored by Git.
