...
# ------------------------------------------------------------
# restrict analysis to recent 2 nonbreeding cycles
# ------------------------------------------------------------
site_location_fixes <- site_location_fixes %>%
  add_nb_year(date)

recent_two_nb_years <- site_location_fixes %>%
  st_drop_geometry() %>%
  distinct(nb_year) %>%
  arrange(nb_year) %>%
  slice_tail(n = 2) %>%
...
site_location_fixes <- site_location_fixes %>%
  filter(nb_year %in% recent_two_nb_years)
...
# save outputs
saveRDS(site_presence_days, "data/recent_2_years/site_presence_days.rds")
saveRDS(site_use_by_year, "data/recent_2_years/site_use_by_year.rds")
saveRDS(site_summary_points, "data/recent_2_years/site_summary_points.rds")
saveRDS(site_location_fixes, "data/recent_2_years/site_location_fixes.rds")
